...
# 4. Construire le modèle de fine tuning à partir de VGG16
base_model = VGG16(weights='imagenet', include_top=False, input_shape=(224, 224, 3))
x = GlobalAveragePooling2D()(base_model.output)
x = Dense(128, activation='relu')(x)  # Couche intermédiaire facultative
predictions = Dense(2, activation='softmax')(x)  # 2 classes : 0 et 1

model = Model(inputs=base_model.input, outputs=predictions)

# Option : geler les couches du modèle de base pour commencer
...
# 5. Entraîner le modèle avec les images pseudo‑labellisées
...
model.save("modele_finetune_1.h5")
